...
def main():
    ...
    for row in rows:
        ...
        # gpt-5.6-luna emits malformed JSON on 1-2 products per run, a different
        # one each time. Two shapes seen: a stray `,"` before the closing brace,
        # and an ORPHANED string where a key should be (737956 in the
        # full-catalogue run). Unrepaired the product is dropped from every
        # analysis and reads as empty -- which once made a correct extraction
        # look like total content loss.
        fields, note = parse_booking_json(
            row["response"]["body"]["choices"][0]["message"]["content"])
        if fields is None:
            print(f"  !! UNPARSEABLE, skipped: {pid}")
            continue
        if note:
            repaired.append(f"{pid}({note})")
